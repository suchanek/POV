...
#declare rad = 38.0;              // 31 units in radius for the sweep
#declare half_ang = 90.0;         
#declare sslope = -2.0 * half_ang; // change in -12 degrees over the range.
#declare ypos = 6;
#declare sslope = -2.0 * half_ang; // change in -12 degrees over the range.
#declare initial_ang = 0.0;
#declare cpoint_at = <0,-1,0>;

#declare theta = clock * sslope + half_ang + initial_ang;     // equation for theta as a fxn of clock
...
#declare rised_labeled = union {
 ...
 //object { risedronate_text translate <-5, -15.5, -1> }
 object { png_text translate <-5, 13.5, -0> }
}                                    
